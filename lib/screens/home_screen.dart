...
class _CaseMatch {
  ...
  const _CaseMatch(this.caseFile,
      {this.hitPersonName, this.personHits = const []});
}
      ...
      child: InkWell(
        ...
        onLongPress: () async {
          ...
          if (action == 'delete') {
            ...
            if (ok == true) {
              AppState.instance.cases
                  .removeWhere((c) => c.id == caseFile.id);
              await AppState.instance.persist();
            }
            ...
            if (!context.mounted) return;
            final tagsC =
                TextEditingController(text: caseFile.tags.join(', '));
            final ok = await showDialog<bool>(
              context: context,
              builder: (ctx) => AlertDialog(
                title: Text(tr('edit_case_tags')),
